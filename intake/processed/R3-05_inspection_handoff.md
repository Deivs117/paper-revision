...
## 0. Instrucciones al equipo de redacción

**PROGRESS.md:** R3-05 | **Owner:** Deivi | **Status:** drafted  
**Sección destino:** `results.tex` — nueva subsección dentro de Simulation Results (tras las suites A/B/C/Complex existentes, antes de Physical Implementation Results)

### Qué producir

Una **subsección breve**. No verboso — una tabla que resuma los 20 ensayos es demostración suficiente; no se necesita figura generada por script si la tabla cumple. Decidir en equipo si tipear la tabla completa de §9.6 o un subconjunto de columnas.

### Correspondencia reviewer → métrica

| Comentario del reviewer | Qué reportar | Referencia en este doc |
|---|---|---|
| *"target tracking accuracy"* | `d_final`: μ=1.076 m, σ=0.038 m (17 trials exitosos) | §9.2 |
| *"coverage rate"* | No medida — sustituida por SR=85 %; argumento completo | §1.1 |
| *"autonomous re-planning after obstacle blockage"* | Ratio N\_lidar/N\_mode\_X ≈ 2.3× — re-enrutamiento reactivo emergente, no deliberado | §9.4 |

### Reglas de estilo

- Todo texto nuevo en `\revblue{}`
- Declarar las cuatro limitaciones de §8 con honestidad (sin path planning, sin coverage deliberada, primera adquisición no re-adquisición)
- Si se hace tabla LaTeX con los 20 ensayos: columnas sugeridas = Trial, Verdict, `d_final` (m), `N_lidar`, `N_mode_X`, `T_acq` (s), `sim_time` (s) — datos individuales en §9.3
- Análisis de los 3 fallos (§9.5) cabe en 2–3 líneas de prosa, no necesita tabla separada
