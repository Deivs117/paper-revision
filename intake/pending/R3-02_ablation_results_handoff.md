...
1. Se implementó un parámetro `ablation_mode` en `red_neuronal.py` (rama `Deiv` de
   `PETER_SIMULATION`, commit `e29d9a161e65209051e5508b7d6bff0cb20cdbcd`) con 3 variantes: `full`
   (control), `no_lateral_inhibition` (quita la inhibición cruzada entre canales del circuito
   STN/GPi/GPe/STR), `threshold_only` (bypasea el circuito completo por una lógica de umbral de
   prioridad fija, análoga a la que el propio Módulo de Locomoción ya usa en otro punto del diseño).
2. Se corrieron 72 simulaciones (3 variantes × 2 escenarios × 12 réplicas) vía el orquestador
   ...
   de escenario ya publicados en `tab:consolidated_simulation_metrics` de `results.tex`.
...
- `experiments/R3-02-wta-vs-threshold/data/` — los 72 `trial_summary.json`/`metrics_raw.csv`/
...
- `experiments/R3-02-wta-vs-threshold/output/ablation_stats_summary.{json,csv}` — el análisis
  estadístico ya calculado (committeado, es pequeño).

## Resultado (resumen — el detalle completo con tablas está en el README del experimento)

Sin conflicto (Appetitive), las 3 variantes rinden igual (100%/91.7%/91.7% de éxito, ninguna
diferencia significativa) — la ablación no rompe el caso simple, como se esperaba.

Con conflicto real (Complex Navigation):
- `full` → 100% de éxito.
- `no_lateral_inhibition` → 25% de éxito (Fisher exact p=0.00034 vs. `full` — **muy significativo**).
- `threshold_only` → 83.3% de éxito (p=0.478, no significativo en esta métrica sola con n=12) —
  **pero** sí muestra degradación significativa en latencia de decisión (p=0.014), `roll_rms`
  (p=0.017) y `pitch_rms` (p=0.0029) frente a `full`.

**Limitación conocida:** λ (eficiencia de conflicto) sale 1.0 en las 72 corridas por una limitación
