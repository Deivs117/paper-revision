...
%
% NOTE: the "hand-coded threshold logic" row/citation below was corrected under C-13
% (originally filed as "C-07", then "C-09" -- both collided with a concurrent
% contributor's row of the same ID; see PROGRESS.md's coherence-audit note) after
% web-validating each cited paper's full text against the
% claim: \citet{Bjelonic2019KeepRollin}'s own experiments describe mode transitions
% as commanded by a human operator via joystick ("The robot is driven using external
% velocity inputs coming from a joystick"), not by an autonomous hand-coded threshold
% -- so it was dropped from this row (it remains correctly cited elsewhere in
% introduction.tex for its actual contribution, the whole-body motion controller).
% \citet{Kashiri2019CENTAURO} was kept -- secondary sources describe CENTAURO's mode
% choice as tied to a terrain-flatness heuristic, consistent with this row's claim.

[...existing paragraph text unchanged up to:] locomotion mode switching is resolved either heuristically through hand-coded threshold logic \revblue{\citep{Kashiri2019CENTAURO}}, by model predictive control \citep{bjelonic2019,Bjelonic2022OfflineMotion}, or by end-to-end RL policies [...]
